    ...
    return (
        <Base $width="45rem" $justifyContent="space-between" $flexDirection="row" $padding="2rem 2rem" $gap={4} $boxShadow $borderRadius={16} >
            <Base $width="fit-content" $height="max-content" $gap={2}>
                ...
                <BaseText>Avaliação do vendedor: <BaseText $fontWeight="bold" $fontSize={32} $color={props.vendor.rating >= 70 ? "#09e409a6" : "#fb0" }>{props.vendor.rating}</BaseText></BaseText>
            </Base>
            <Base $gap={2} $height="max-content" $maxWidth="100%">
                ...
                <BaseText>Estoque: {props.stock}</BaseText>
                <BaseText $color="#09e409a6" $fontSize={36} $fontWeight="bold">R$ {props.price.toFixed(2)}</BaseText>
                <BaseButton $width="100%" $height="4rem"><BaseText $fontSize={16} $color="#fff" $fontWeight="bold">Comprar</BaseText></BaseButton>
            </Base>
            <Base $width="fit-content" $gap={2}>
                {favorite ? 
                <FaHeart cursor={"pointer"} size={32} color="#f04" onClick={() => setFavorite(!favorite)} /> :
                <FaRegHeart cursor={"pointer"} size={32} onClick={() => setFavorite(!favorite)}/>    
            }
